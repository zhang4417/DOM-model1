...
var div1 = dom.create('<div>新的DIV</div>');
console.log(div1);
console.log(box); //dom.after(div1, test)
//dom.before(div1, test)

dom.append(div1, box); //dom.wrap(div1, test)
//dom.remove(test)
...
dom.class.add(test, 'sx'); //dom.class.remove(test, 'sx')

console.log(dom.class.contains(test, 'sx'));

var fn = function fn() {
  return console.log("点击一下");
};

dom.on(test, 'click', fn);
dom.off(test, 'click', fn);
var boxDiv = dom.get('#box')[0];
console.log(boxDiv);
var testDiv = dom.get('#test2', boxDiv)[0];
console.log(testDiv);
console.log(dom.siblings(test));
console.log(dom.next(test));
console.log(dom.previous(test));
dom.each(dom.children(box), function (i) {
  return dom.style(i, 'color', 'red');
});
